...
func (this *SockSrv) respConn(conn net.Conn) {
	...
	for {
		...
		if buf[0] == 0x01 {
			uhash = login(buf, protocol, conn, false, this.db)//chigh_id, cport, uhash = login(buf, protocol, conn, this.Debug, this.db)
		} else if buf[0] == 0x14 {
			...
			filesources(buf, protocol, conn, this.Debug, buflen, this.db)
		} else if buf[0] == 0x1c {
